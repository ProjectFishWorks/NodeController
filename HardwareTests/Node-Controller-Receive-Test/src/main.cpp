...
twai_message_t stop_resp;

twai_message_t data_message;
...
static void twai_transmit_task(void *arg)
{
    while (1)
    {
        ...
        if (action == TX_SEND_PING_RESP)
        {
            ...
            Serial.println("Transmitted ping response");
            xSemaphoreGive(ctrl_task_sem);
        }
...
void setup()
{
    ...
    ping_resp.identifier = ID_SLAVE_PING_RESP;
    ping_resp.data_length_code = 0;
    ping_resp.data[0] = 0;
    ping_resp.extd = 0;
    ping_resp.rtr = 0;
    ping_resp.ss = 0;
    ping_resp.self = 0;
    ping_resp.dlc_non_comp = 0;

    stop_resp.identifier = ID_SLAVE_STOP_RESP;
    stop_resp.data_length_code = 0;
    stop_resp.data[0] = 0;
    stop_resp.extd = 0;
    stop_resp.rtr = 0;
    stop_resp.ss = 0;
    stop_resp.self = 0;
    stop_resp.dlc_non_comp = 0;

    data_message.identifier = ID_SLAVE_DATA;
    data_message.data_length_code = 4;
    data_message.data[0];
    data_message.data[1];
    data_message.data[2];
    data_message.data[3];
